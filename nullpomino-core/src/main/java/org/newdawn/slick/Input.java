...
public class Input{
	...
	public void initControllers() throws SlickException{
		...
		try{
			...
			for(int i=0;i<controllers.size();i++) Log.info(i+" : "+((Controller)controllers.get(i)).getName());
	...
	public void considerDoubleClick(int button,int x,int y){
		...
		}else if(clickButton==button) if((System.currentTimeMillis()<doubleClickTimeout)){
	...
	public void poll(int width,int height){
		...
		for(Object o : keyListenersToAdd) addKeyListenerImpl((KeyListener)o);
		keyListenersToAdd.clear();
		for(Object o : mouseListenersToAdd) addMouseListenerImpl((MouseListener)o);
		mouseListenersToAdd.clear();

		if(doubleClickTimeout!=0) if(System.currentTimeMillis()>doubleClickTimeout) doubleClickTimeout=0;

		this.height=height;

		for(Object listener2 : allListeners){
			ControlledInputReciever listener=(ControlledInputReciever)listener2;
		...
		while(Keyboard.next()) if(Keyboard.getEventKeyState()){
			...
			consumed=false;
			for(Object listener1 : keyListeners){
				KeyListener listener=(KeyListener)listener1;
		...
		while(Mouse.next()) if(Mouse.getEventButton()>=0) if(Mouse.getEventButtonState()){
			...
			for(Object listener1 : mouseListeners){
				MouseListener listener=(MouseListener)listener1;
				if(listener.isAcceptingInput()){
			...
			if(Mouse.isGrabbed()&&displayActive) if((Mouse.getEventDX()!=0)||(Mouse.getEventDY()!=0)){
				consumed=false;
				for(Object listener1 : mouseListeners){
					MouseListener listener=(MouseListener)listener1;
					if(listener.isAcceptingInput()){
		...
		if(controllersInited) for(int i=0;i<getControllerCount();i++){
		...
		if(keyRepeat) for(int i=0;i<1024;i++)
			if(pressed[i]&&(nextRepeat[i]!=0)) if(System.currentTimeMillis()>nextRepeat[i]){
		...
		for(Object listener1 : allListeners){
			ControlledInputReciever listener=(ControlledInputReciever)listener1;
			listener.inputEnded();
		}
	...
	private void fireControlPress(int index,int controllerIndex){
		consumed=false;
		for(Object listener1 : controllerListeners){
