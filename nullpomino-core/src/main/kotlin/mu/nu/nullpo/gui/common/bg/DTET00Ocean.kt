...
//				Sea(I) = Sea(I) + (I - 2.5) * (0.03 - (TrM > 0) * 0.08)
	...
	override fun draw() {
		rasterX.zip(waveX).forEachIndexed {i, it ->
			if(i==0) {
				val x = it.first
				//sky
				...
				val wy = i*8+((it.second/waveMag(i))-1)*i*waveH
				val x = (it.first+it.second).let {it+if(it<0) 640 else 0}%640
				img.draw(0f, 240f+i*8, x, 240+wy, 640f, minOf(248+wy, 480f))
				img.draw(640-x, 240f+i*8, 0f, 240+wy, x, minOf(248+wy, 480f))
				/*
		With Src
