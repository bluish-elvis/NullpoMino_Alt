...
from mu.nu.nullpo.game.component import Field, Statistics
...
class BasePvPAI(PyAI):
  ...
  def renderState(self, engine, playerID):
    ...
    r.drawScoreFont(engine, 19, 39, self.name.upper(), EventReceiver.COLOR_GREEN, 0.5)
    r.drawScoreFont(engine, 24, 40, "X", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 27, 40, "Y", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 30, 40, "RT", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 19, 41, "BEST:", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 24, 41, String.valueOf(self.bestX), 0.5)
    r.drawScoreFont(engine, 27, 41, String.valueOf(self.bestY), 0.5)
    r.drawScoreFont(engine, 30, 41, String.valueOf(self.bestRt), 0.5)
    r.drawScoreFont(engine, 19, 42, "SUB:", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 24, 42, String.valueOf(self.bestXSub), 0.5)
    r.drawScoreFont(engine, 27, 42, String.valueOf(self.bestYSub), 0.5)
    r.drawScoreFont(engine, 30, 42, String.valueOf(self.bestRtSub), 0.5)
    r.drawScoreFont(engine, 19, 43, "NOW:", EventReceiver.COLOR_BLUE, 0.5)
    if engine.nowPieceObject is None:
      r.drawScoreFont(engine, 24, 43, "-- -- --", 0.5)
    else:
      r.drawScoreFont(engine, 24, 43, String.valueOf(engine.nowPieceX), 0.5)
      r.drawScoreFont(engine, 27, 43, String.valueOf(engine.nowPieceY), 0.5)
      r.drawScoreFont(engine, 30, 43, String.valueOf(engine.nowPieceObject.direction), 0.5)
    r.drawScoreFont(engine, 19, 44, "REWARD:", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 27, 44, String.valueOf(self.lastReward),
                    EventReceiver.COLOR_WHITE if self.lastReward == 0 else (
                      EventReceiver.COLOR_GREEN if self.lastReward > 0 else EventReceiver.COLOR_RED), 0.5)
    r.drawScoreFont(engine, 19, 45, "QVALUE:", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 27, 45, String.valueOf(self.lastqvalue),
                    EventReceiver.COLOR_WHITE if self.lastqvalue == 0 else (
                      EventReceiver.COLOR_GREEN if self.lastqvalue > 0 else EventReceiver.COLOR_RED), 0.5)
    r.drawScoreFont(engine, 19, 46, "CHOICE:", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 27, 46, String.valueOf(self.lastExploreChoice),
                    EventReceiver.COLOR_RED if self.lastExploreChoice == "EXPLORE" else EventReceiver.COLOR_GREEN, 0.5)
    r.drawScoreFont(engine, 19, 47, "HOLE  :", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 27, 47, String.valueOf(self.lastHole),
                    EventReceiver.COLOR_RED if self.lastHole > 0 else EventReceiver.COLOR_WHITE, 0.5)
    r.drawScoreFont(engine, 19, 48, "THINK ACTIVE:", EventReceiver.COLOR_BLUE, 0.5)
    r.drawScoreFont(engine, 32, 48, GeneralUtil.getOorX(self.thinking), 0.5)
    # r.drawScoreFont(engine, 19, 46, "IN ARE:", EventReceiver.COLOR_BLUE, 0.5)
    # r.drawScoreFont(engine, 26, 46, GeneralUtil.getOorX(inARE), 0.5)
    pass

  def setControl(self, engine, ctrl):
    if ((not engine.nowPieceObject is None) and (engine.stat == GameEngine.Status.MOVE) and (
        self.delay >= engine.aiMoveDelay) and (engine.statc[0] > 0) and
        (not engine.aiUseThread or (
            self.threadRunning and not self.thinking and (self.thinkCurrentPieceNo <= self.thinkLastPieceNo)))):
        ...
        if ((self.bestX < minX - 1) or (self.bestX > maxX + 1) or (self.bestY < nowY)) and (rt == self.bestRt):
          ...
          if nowX > self.bestX:
            ...
            if (self.bestRtSub == -1) and (self.bestX == self.bestXSub):
              ...
              if (engine.ruleopt.harddropEnable and not engine.ruleopt.harddropLock and not ctrl.isPress(
                  ctrl.BUTTON_UP)):
                input |= ctrl.BUTTON_BIT_UP
  ...
  def thinkMain(self, engine, playerID):
    ...
    holdChance = self.HOLD_EPSILON if holdPiece is not None else 0
    nowState = self.computeState(engine.field, nowPiece, nextPiece)
    holdState = self.computeState(engine.field, holdPiece, nextPiece) if holdPiece is not None else nowState
    ...
    if self.lastState is not None and self.lastAction is not None:
      ...
      self.qtable[self.lastState][self.lastAction] = curQ + self.ALPHA * (
          reward + self.GAMMA * (bestQOfNowAndHoldState - curQ))
    ...
    if IS_DEBUG:
      print(state)
    self.thinkState(engine, state, nowPiece, nextPiece, isHold)

  def thinkState(self, engine, state, nowPiece, nextPiece, isHold):
    fld = engine.field
    possibleAction = {}
